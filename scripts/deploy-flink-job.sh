...
JOB_ID=$(docker compose -f compose.dev.yml exec flink-jobmanager flink list | grep -E "RUNNING|INITIALIZING" | awk '{print $4}' | head -n 1 | tr -d '\r\n')
...
if [ -n "$JOB_ID" ]; then
    ...
    SAVEPOINT_LINE=$(docker compose -f compose.dev.yml exec flink-jobmanager flink stop "$JOB_ID" --savepointPath /opt/flink/state/savepoints)
...
./gradlew streaming:product-ranking-job:clean shadowJar
docker compose -f compose.dev.yml build flink-jobmanager flink-taskmanager

# 3. Spin the infrastructure up with the updated execution command parameters
echo "Launching Flink topology cluster nodes..."
docker compose -f compose.dev.yml up -d --force-recreate flink-jobmanager flink-taskmanager
